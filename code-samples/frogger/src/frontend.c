...
void print_rectangle(int top_y, int bottom_y, int left_x, int right_x) {
  MVADDCH(top_y, left_x, ACS_ULCORNER); // курсор в верхнее левое сост ставим вертикальную полупалочку

  int i = left_x + 1; // точка отсчета от первой точки 

  for (; i < right_x; i++) MVADDCH(top_y, i, ACS_HLINE); // проставляем минусы до right_x
  MVADDCH(top_y, i, ACS_URCORNER);

  for (int i = top_y + 1; i < bottom_y; i++) {
    MVADDCH(i, left_x, ACS_VLINE);
    MVADDCH(i, right_x, ACS_VLINE); // так же вниз | to bottom_y
  }
